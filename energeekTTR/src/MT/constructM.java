...
public class constructM extends pagerank {
	...
	public double worddivider(String s,int dataid,ArrayList<PrintWriter> outputlist,ArrayList<String> wholewords,ArrayList<Integer> wholestatic,int totalcount) throws FileNotFoundException{
	   ...
	   /*************************Out Put**************************/
		  ...
		  /*  if(file1.exists()){
		   ...
		   /* PrintWriter output = new PrintWriter(file1);*/
	    	 ...
	    	 */
	    	 ...
	    	 int lengthM = 4;//tfidfwordbag.size()-4;
	    ...
	    /*	 System.out.println();
	    	...
	    	/* output2.println(dataid+" :"+"Choose Number "+(maxindex-3) +" Translation."+" "+"BLEU Evaluation:"+sum+" "+"Similirity to Ref1:"+ref1+" "+"Similirity to Ref2:"+
	    			 ref2+"\n "+"Similirity to Ref3:"+ref3+" "+"Similirity to Ref4:"+ref4);
	    	 output2.println("Processed Transation or Edition");
		    	for(int k = 0; k < res.size();k++){
		    		output2.print(res.get(k)+" ");
		    	}
		    	output2.println();
	    	  
	    	  ArrayList<String> real_res = real_sentences.get(maxindex);
		    	 
		    	// System.out.println("Max:"+maxindex+" "+ref1+" "+ref2+" "+ref3+" "+ref4);
	    	  output2.println("Original Transation or Edition");
			    	for(int k = 0; k < real_res.size();k++){
			    		output2.print(real_res.get(k)+" ");
			    	}
			    	output2.println();
	...
	public void getM(String filename,ArrayList<String> wholewords,ArrayList<Integer> wholestatic,int totalcount) throws IOException{
		...
		ArrayList<PrintWriter> outputlist = new ArrayList<PrintWriter> (); 
		 String filename2 = "F:/NLP/Evaluate/Ref1.xml";
		...
		/*while(input.hasNext() && i < 1){
		    ...
		    String add = "";
		    int counttab = 0;System.out.println(tmp);
		    if(tmp == "\t" && counttab < 3){
		    	counttab ++;
		    }
		    else if(counttab >= 7 && counttab %2 ==1 && tmp != "\t") continue;
		    else if(counttab >= 3 && tmp != "\t"){
		    	
		    	//String t = input.next();
		    	int index = wordlist.indexOf((String)tmp);
		    	if (index == -1) wordlist.add(tmp);
		    	 add += input.next();
	...
	/* String filename2 = "F:/NLP/newdata.txt";
		...
		while( data!=null  ){
		  ...
		  if (data.indexOf("N/A") == -1 && data.indexOf('&') == -1 && counttab(data) == 34)
				output2.println(data);
			  //System.out.println(counttab(data));
		      i++;
