...
public class EditDistGridSearchCoarse extends EditDist {

	// wordnet relation counter, to log stat of each one fired for the whole corpus
	private static int c_hypernym = 0, c_hyponym = 0, c_synonym = 0, c_entails = 0, c_causing = 0, 
			c_memberOf = 0, c_haveMember = 0, c_substancesOf = 0, c_haveSubstance = 0, c_partsOf = 0, 
			c_havePart = 0, c_sameSynset = 0, c_derived;

	private static HashSet<String> unigramPPset = loadPPs();

	private static int c_node_t1 = 0, c_node_t2 = 0, count_lemma = 0, count_pp = 0;
	...
	// then later durst cant be renamed because 's is already aligned
	// we don't want this to happen: thus instead of assigning 0 cost for stopword alignment,
	// we assign a cost of 2.5.
	...
	boolean[][] canSubstitute; // canSubstitute[i][j] = true iff word1[i] and word2[j] can be substituted according to workdnet

	// substitution cost grid search related
	float[][] subCost;
	float c_pos, c_dep, c_lemma, c_wn, c_pp;

	// the direction, if set to true, then assume tree 2 is the answer tree
	// otherwise assume tree 1 is the answer tree.
	boolean q2a;
	...
	public EditDistGridSearchCoarse(boolean normalized) {
		this(1, 1, 1, normalized);
		this.q2a = true;
	}

	public EditDistGridSearchCoarse(boolean normalized, boolean q2a) {
		this(1, 1, 1, normalized);
		this.q2a = q2a;
	}

	public EditDistGridSearchCoarse(boolean normalized, boolean q2a, float  c_pos, float c_dep, float c_lemma, float c_wn, float c_pp) {
		this(normalized, q2a);
		this.c_pos = c_pos;
		this.c_dep = c_dep;
		this.c_lemma = c_lemma;
		this.c_wn = c_wn;
		this.c_pp = c_pp;
	...
	private static HashSet<String> loadPPs() {
		HashSet<String> unigramPPset = new HashSet<String>();
		try {
			BufferedReader f = FileManager.getReader("resources/paraphrase/unigram.pp.gz");
			String line, pp1, pp2;
			String[] splits;
			while ((line=f.readLine()) != null) {
				splits = line.split("\\s+");
				pp1 = splits[0]; pp2 = splits[1];
				unigramPPset.add(pp1+" "+pp2);
				unigramPPset.add(pp2+" "+pp1);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public EditDistGridSearchCoarse(double ins, double del, double update, boolean normalized) {
	...
	public double nonNormalizedTreeDist(LblTree t1, LblTree t2) {
		...
		c_node_t1 += lbl1.length;
		c_node_t2 += lbl2.length;
		canSubstitute = new boolean[nc1][nc2];
		subCost = new float[nc1][nc2];
		for (int i=0; i<nc1; i++) {
			for (int j=0; j<nc2; j++) {
				if (i == 0 || j == 0) {
					// 0's are root node
					canSubstitute[i][j] = true;
					subCost[i][j] = 0;
				} else {
					if (!lemma1[i].equals(lemma2[j])) {
						canSubstitute[i][j] = false;
						subCost[i][j] +=  this.c_lemma;
					} else {
						canSubstitute[i][j] = true;
						count_lemma += 1;
					}
					if (!pos1[i].equals(pos2[j]))
						subCost[i][j] +=  this.c_pos;
					if (!rel1[i].equals(rel2[j]))
						subCost[i][j] +=  this.c_dep;

					if (!unigramPPset.contains(word1[i] + " " + word2[j])) {
						canSubstitute[i][j] = false;
						subCost[i][j] +=  this.c_pp;
					} else {
						canSubstitute[i][j] = true;
						count_pp += 1;
					}
		...
		if (q2a) {
			for (int j=1; j<nc2; j++) {
				if (!pos2[j].startsWith("nn") && !pos2[j].startsWith("vb") &&
						!pos2[j].startsWith("jj") && !pos2[j].startsWith("rb")) continue;
				//HashSet<String> hypernyms = WordNet.getAllHypernyms(word2[j], pos2[j]);
				//HashSet<String> synonyms = WordNet.getAllSynonyms(word2[j], pos2[j]);
				//HashSet<String> hyponyms = WordNet.getAllHyponyms(word2[j], pos2[j]);
				HashSet<String> hypernyms = WordNet.getHypernymsSet(word2[j], pos2[j]);
				HashSet<String> synonyms = WordNet.getSynonymsSet(word2[j], pos2[j]);
				HashSet<String> hyponyms = WordNet.getHyponymsSet(word2[j], pos2[j]);
				HashSet<String> derived = WordNet.getDerivedSet(word2[j], pos2[j]);
				HashSet<String> entails = null;
				HashSet<String> causing = null;
				HashSet<String> membersOf = null;
				HashSet<String> HaveMember = null;
				HashSet<String> substancesOf = null;
				HashSet<String> HaveSubstance = null;
				HashSet<String> partsOf = null;
				HashSet<String> HavePart = null;
				if (pos2[j].startsWith("vb")) {
					entails = WordNet.getEntailingSet(word2[j]);
					causing = WordNet.getCausingSet(word2[j]);
				}
				if (pos2[j].startsWith("nn")) {
					membersOf = WordNet.getMembersOfSet(word2[j]);
					substancesOf = WordNet.getSubstancesOfSet(word2[j]);
					partsOf = WordNet.getPartsOfSet(word2[j]);
					HaveMember = WordNet.getHaveMemberSet(word2[j]);
					HaveSubstance = WordNet.getHaveSubstanceSet(word2[j]);
					HavePart = WordNet.getHavePartSet(word2[j]);
				}
				for (int i=1; i<nc1; i++) {
					if (lemma1[i].equals(lemma2[j]))
						canSubstitute[i][j] = true;
					// requires that both words have to be nouns or verbs
					if (!pos1[i].startsWith("nn") && !pos1[i].startsWith("vb") &&
							!pos1[i].startsWith("rb") && !pos1[i].startsWith("jj")) continue;
					if (!pos2[j].subSequence(0, 2).equals(pos1[i].subSequence(0, 2))) continue;
					if ((hypernyms != null && hypernyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
						c_hypernym += 1;
					} else if ((synonyms != null && synonyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
						c_synonym += 1;
					} else if ( (hyponyms != null && hyponyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
						c_hyponym += 1;
						//subCost[i][j] +=  this.c_wn;
					} else if (derived != null && !lemma1[i].equals(lemma2[j]) && derived.contains(word1[i])) {
						c_derived += 1;
						//System.out.println(word1[i] + " " + word2[j]);
					} else if (pos2[j].startsWith("vb")) {
						if (entails != null && entails.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_entails += 1;
						} else if (causing != null && causing.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_causing += 1;
						}  else if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
							//subCost[i][j] +=  this.c_wn;
							c_sameSynset += 1;
						} else {
							subCost[i][j] +=  this.c_wn;
						...
						if (membersOf != null && membersOf.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_memberOf += 1;
						} else if (substancesOf != null && substancesOf.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_substancesOf += 1;
						} else if (partsOf != null && partsOf.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_partsOf += 1;
						} else if (HaveMember != null && HaveMember.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_haveMember += 1;
						} else if (HaveSubstance != null && HaveSubstance.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_haveSubstance += 1;
						} else if (HavePart != null && HavePart.contains(word1[i])) {
							canSubstitute[i][j] = true;
							c_havePart += 1;
							//subCost[i][j] +=  this.c_wn;
						}  else if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
			...
			// tree 1 is the answer tree
			for (int i=1; i<nc1; i++) {
				if (!pos1[i].startsWith("nn") && !pos1[i].startsWith("vb") &&
						!pos1[i].startsWith("jj") && !pos1[i].startsWith("rb")) continue;
				//HashSet<String> hypernyms = WordNet.getAllHypernyms(word1[i], pos1[i]);
				//HashSet<String> synonyms = WordNet.getAllSynonyms(word1[i], pos1[i]);
				//HashSet<String> hyponyms = WordNet.getAllHyponyms(word1[i], pos1[i]);
				HashSet<String> hypernyms = WordNet.getHypernymsSet(word1[i], pos1[i]);
				HashSet<String> synonyms = WordNet.getSynonymsSet(word1[i], pos1[i]);
				HashSet<String> hyponyms = WordNet.getHyponymsSet(word1[i], pos1[i]);
				HashSet<String> derived = WordNet.getDerivedSet(word1[i], pos1[i]);
				HashSet<String> entails = null;
				HashSet<String> causing = null;
				HashSet<String> membersOf = null;
				HashSet<String> HaveMember = null;
				...
				if (pos1[i].startsWith("vb")) {
					entails = WordNet.getEntailingSet(word1[i]);
					causing = WordNet.getCausingSet(word1[i]);
				}
				if (pos1[i].startsWith("nn")) {
					membersOf = WordNet.getMembersOfSet(word1[i]);
					substancesOf = WordNet.getSubstancesOfSet(word1[i]);
					partsOf = WordNet.getPartsOfSet(word1[i]);
					HaveMember = WordNet.getHaveMemberSet(word1[i]);
					HaveSubstance = WordNet.getHaveSubstanceSet(word1[i]);
					HavePart = WordNet.getHavePartSet(word1[i]);
				}
				for (int j=1; j<nc2; j++) {
					if (lemma1[i].equals(lemma2[j]))
						canSubstitute[i][j] = true;
					// requires that both words have to be nouns or verbs
					if (!pos2[j].startsWith("nn") && !pos2[j].startsWith("vb") &&
							!pos2[j].startsWith("rb") && !pos2[j].startsWith("jj")) continue;
					if (!pos2[j].subSequence(0, 2).equals(pos1[i].subSequence(0, 2))) continue;
					if (hypernyms != null && hypernyms.contains(word2[j]))  {
						canSubstitute[i][j] = true;
						c_hypernym += 1;
					} else if (synonyms != null && synonyms.contains(word2[j])) {
						canSubstitute[i][j] = true;
						c_synonym += 1;
					} else if (hyponyms != null && hyponyms.contains(word2[j])) {
						canSubstitute[i][j] = true;
						c_hyponym += 1;
					} else if (derived != null && !lemma1[i].equals(lemma2[j]) && derived.contains(word2[j])) {
						c_derived += 1;
						//System.out.println(word2[j] + " " + word1[i]);
					} else if (pos1[i].startsWith("vb")) {
						if (entails != null && entails.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_entails += 1;
						} else if (causing != null && causing.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_causing += 1;
						}  else if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
							//subCost[i][j] +=  this.c_wn;
						...
						if (membersOf != null && membersOf.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_memberOf += 1;
						} else if (substancesOf != null && substancesOf.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_substancesOf += 1;
						} else if (partsOf != null && partsOf.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_partsOf += 1;
						} else if (HaveMember != null && HaveMember.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_haveMember += 1;
						} else if (HaveSubstance != null && HaveSubstance.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_haveSubstance += 1;
						} else if (HavePart != null && HavePart.contains(word2[j])) {
							canSubstitute[i][j] = true;
							c_havePart += 1;
						}  else if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
							//subCost[i][j] +=  this.c_wn;
		...
		//			for (int j=1; j<subCost[i].length; j++) {
		//				if (subCost[i][j] != 0)
		//					subCost[i][j] = Float.MAX_VALUE;
		//			}
		//		}
	...
	public static void printStat() {
		System.out.println(String.format("hypernym: %d\nhyponym: %d\nsynonym: %d\n" +
				"entails: %d\ncausing: %d\nmemberOf: %d\nhaveMember: %d\n" +
				"substanceOf: %d\nhaveSubstance: %d\npartsOf: %d\nhaveParts: %d\nsameSynset: %d\n" +
				"derived: %d\n",
				c_hypernym, c_hyponym, c_synonym, c_entails, c_causing, 
				c_memberOf, c_haveMember, c_substancesOf, c_haveSubstance, c_partsOf, 
				c_havePart, c_sameSynset, c_derived
				));

		System.out.println("lemma match: " + count_lemma);
		System.out.println("pp match: " + count_pp);

		System.out.println(String.format("total length of tree1: %d\ntotal length of tree2: %d",
				c_node_t1, c_node_t2));
