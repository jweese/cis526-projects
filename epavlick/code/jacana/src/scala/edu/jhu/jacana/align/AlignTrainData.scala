...
class AlignTrainData (fname:String = "alignment-data/msr/converted/RTE2_tiny_M.align.txt", 
        transpose: Boolean = false, tokenize: Boolean = true) extends TrainAlignData {
	private val list: List[AlignPair] = 
	    if (fname.contains("json")) new ShallowJsonReader(fname, transpose, tokenize).toList
	    else new MsrReader(fname, transpose).toList
	private val trainList = for (x <- list) yield new AlignTrainRecord(x)
