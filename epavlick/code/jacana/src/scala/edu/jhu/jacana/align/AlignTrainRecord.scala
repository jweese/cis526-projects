...
class AlignTrainRecord (pair: AlignPair) extends TrainAlignRecord {
    ...
    if (!AlignerParams.phraseBased) {
	    for (i <- 0 until numTokens) {
	         pair.alignMatrix(i).toList.indexOf(1) match {
	            ...
	            case m if m > -1 => {
                	...
                	labelsPerToken(i) = m + 1; IndexLabelAlphabet.get((m+1))
   	                src2tgtCount(1)(1) += 1
	            }
	        }       
        ...
        while (i < pair.srcLen) {
            ...
            if (allZeros) {
                ...
                var stateID = IndexLabelAlphabet.getMergedStateIdx(j+1, pair.tgtLen, maxJ-j)
                for (ii <- i until maxI)
    ...
    def y_length(): Int = { 
        if (!AlignerParams.phraseBased)
        	...
        	return IndexLabelAlphabet.getMaxMergedStateIdx(pair.tgtLen)
    }
