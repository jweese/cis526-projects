...
object FlatTokenAligner {
    def main(args: Array[String]): Unit = {
        ...
        if (aligner.trainFilename == null && !aligner.eclipseMode)
			...
			val trainData = new AlignTrainData(aligner.trainFilename, aligner.transpose)
			IndexLabelAlphabet.freeze()
			
			var devData:AlignTrainData = if (aligner.devFilename!=null) new AlignTrainData(aligner.devFilename, aligner.transpose) else null
        ...
        if (aligner.alignFilename != null) {
			...
			val testData = new AlignTrainData(aligner.testFilename, aligner.transpose)
