...
abstract class AlignFeature  extends Serializable {
     ...
     * 
     */
    def addTokenBasedFeature(pair: AlignPair, ins:AlignFeatureVector, i:Int, j:Int, currState:Int, featureAlphabet: Alphabet) {
        addPhraseBasedFeature(pair, ins, i, 1, j, 1, currState, featureAlphabet)
    }
     ...
     * 
     */
    def extract(record: AlignSequence, featureAlphabet: Alphabet):Array[AlignFeatureVector] = {
        ...
        if (!AlignerParams.phraseBased) {
	        ...
	        while (i < pair.srcLen) {
	            ...
	            var j = -1
	            while (j < pair.tgtLen) {
	                addTokenBasedFeature(pair, ins, i, j, j+1, featureAlphabet)
	                j += 1
	            }
	            i += 1
	        }
	                ...
	                while (srcSpan <= Math.min(AlignerParams.maxSourcePhraseLen, i+1)) {
		                ...
		                if (j == -1) {
		                   ...
		                   if (srcSpan == 1 && tgtSpan == 1)
		                	   addTokenBasedFeature(pair, ins, i, -1, NULL_STATE, featureAlphabet)
	                	   else
	                		   addPhraseBasedFeature(pair, ins, i-srcSpan+1, srcSpan, -1, 1, NULL_STATE, featureAlphabet)
		                } else {
			                while (tgtSpan <= Math.min(AlignerParams.maxTargetPhraseLen, j+1)) {
			                	...
			                	val state = IndexLabelAlphabet.getMergedStateIdx(posJ+1, pair.tgtLen, tgtSpan)
			                   ...
			                   if (srcSpan == 1 && tgtSpan == 1)
		                		   ...
		                		   addPhraseBasedFeature(pair, ins, i-srcSpan+1, srcSpan, posJ, tgtSpan, state, featureAlphabet)
