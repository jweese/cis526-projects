 ...
 *
 */
class ShallowJsonReader(fname: String, transpose: Boolean = false, tokenize: Boolean = true) extends Iterator[AlignPair] {
    ...
    def hasNext() : Boolean = {
        ...
        if (current < l.length) {
            val a = l(current)
            if (!transpose)
	            pair = new AlignPair(a.id, a.source, a.target, tokenize)
            else
	            pair = new AlignPair(a.id, a.target, a.source, tokenize)
...
object ShallowJsonReaderTest {
	def main(args: Array[String]): Unit = {
		val reader = new ShallowJsonReader("alignment-data/edinburgh/gold.train.sure.json", false)
